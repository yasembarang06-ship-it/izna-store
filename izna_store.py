...
def main():
    ...
    with st.sidebar:
        ...
        # Menu
        menu = st.radio("📌 Menu", ["🏠 Beranda", "🛒 Keranjang", "📦 Pesanan Saya", "⚙️ Admin"])
        ...
        if st.session_state.cart:
            ...
            st.success(f"🛒 Keranjang: {total_items} item\n\nTotal: {format_rupiah(total_harga)}")
        else:
    ...
    if menu == "🏠 Beranda":
        ...
        <div style="background: linear-gradient(90deg, #667eea, #764ba2); padding: 30px; border-radius: 15px; text-align: center; color: white; margin-bottom: 30px;">
            <h1 style="margin: 0;">🛒 Izna Store</h1>
        ...
        if not products:
            st.warning("⚠️ Produk belum tersedia. Silakan tambah di menu Admin.")
        else:
            cols = st.columns(4)
            for i, prod in enumerate(products):
                with cols[i % 4]:
                    with st.container():
                        ...
                        st.markdown(f"💰 **{format_rupiah(prod[4])}**")
                        st.markdown(f"📦 Stok: {prod[6]}")
                        
                        if st.button(f"🛒 Tambah ke Keranjang", key=f"add_{prod[0]}"):
                            add_to_cart(prod)
    ...
    # === HALAMAN KERANJANG ===
        ...
        if not st.session_state.cart:
            ...
            with st.form("checkout_form"):
                ...
                metode = st.radio("Metode:", ["💵 Transfer Bank", "📱 QRIS (QR Code)"])
                ...
                if metode == "💵 Transfer Bank":
                    ...
                    """, unsafe_allow_html=True)
                    bukti = st.file_upload("📤 Upload Bukti Transfer", type=['jpg', 'png', 'jpeg'])
                    ...
                    bukti = st.file_upload("📤 Upload Bukti Pembayaran", type=['jpg', 'png', 'jpeg'])
                
                submit = st.form_submit_button("✅ Pesan Sekarang")
    ...
    # === HALAMAN PESANAN ===
        ...
        orders = get_orders()
        if orders:
            st.markdown("### 📋 Riwayat Pesanan")
            for order in orders:
                status_color = "🟡" if order[9] == "Menunggu" else "🟢"
    ...
    # === HALAMAN ADMIN (FULL KONTROL) ===
        ...
        st.title("⚙️ Panel Admin Izna Store")
        st.markdown("---")
        
        tab1, tab2, tab3 = st.tabs(["📋 Daftar Produk", "➕ Tambah Produk", "📦 Kelola Pesanan"])
        
        # === TAB 1: DAFTAR & EDIT PRODUK ===
        with tab1:
            st.subheader("📋 Kelola Semua Produk")
            ...
            for prod in products:
                with st.expander(f"📦 {prod[2]} - {format_rupiah(prod[4])}"):
                    # Form Edit Produk
